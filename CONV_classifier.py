...
x_train, y_train, x_test, y_test = sd.devide(x_data, y_data, 0.2)
...
W2 = tf.get_variable('W2', shape=[84,10], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
hypothesis = tf.matmul(NNL1, W2) + b2
...
with tf.Session() as sess:
	...
	for i in range(LEARNING_COUNT):
		...
		sess.run(train, feed_dict={X:batch_xs, Y:batch_ys, drop_prob:0.5})
		pg.progress(LEARNING_COUNT, i, sess.run(cost, feed_dict={X:batch_xs, Y:batch_ys, drop_prob:0.5}))
	...
	print "accuracy: ", sess.run(accuracy, feed_dict={X:x_test, drop_prob:1.})

	test_set = np.loadtxt('./data/test.csv', delimiter=',', skiprows= 1)

	test_set = np.reshape(test_set, (len(test_set), 28, 28, 1))
	result = sess.run(hypothesis, feed_dict={X:test_set, drop_prob:1.})
